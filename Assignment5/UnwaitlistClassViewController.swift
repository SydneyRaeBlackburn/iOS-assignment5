//
//  UnwaitlistClassViewController.swift
//  Assignment5
//
...
class UnwaitlistClassViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func unwaitlistButton(_ sender: UIButton) {
        
        // add an alert before unwaitlisting the student from the course
        
        let title = "Delete Course"
        let message = "Are you sure you want to unwaitlist this course?"
        ...
        let unwaitlistAction = UIAlertAction(title: "Unwaitlist", style: .destructive, handler: { (action) -> Void in
            
            // set courseId from the details and unwaitlist the student from the course
            self.courseId = self.details!.id
            self.unwaitlistStudent(redid: self.student!.redid, password: self.student!.password, courseid: self.details!.id)
        })
        
        ac.addAction(unwaitlistAction)
    ...
    /*
     POST method
     creates a URL to send to the server to unwaitlist the student from the course
     */
    func unwaitlistStudent(redid: String, password: String, courseid: Int) {
        ...
        // create URL
        if let url = URL(string: "https://bismarck.sdsu.edu/registration/unwaitlistclass") {
    ...
    /*
     unwaitlists the student from the course
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "showMyClasses"?:
            if segue.destination is MyClassesViewController {
                ...
                // remove unwaitlisted course from the waitlisted classes array
                // also remove the unwaitlisted course details from the details array
                myClassesViewController?.waitlist = self.waitlist!.filter{ $0 != self.courseId }
                var index: Int = 0
                for detail in (self.waitlistDetails)! {
                    if detail.id == self.courseId {
                        self.waitlistDetails!.remove(at: index)
                        break
                    }
                    index += 1
                }
